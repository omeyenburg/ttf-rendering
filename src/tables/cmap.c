...
uint16_t get_cmap_size(unsigned char* buffer, Table* cmap) {
    ...
    u_int16_t version = getUInt16(buffer, cmap->offset);
    if (version != 0) {
        fprintf(stderr, "Expected CMAP version 0, got %i.", version);
        exit(1);
    }

    u_int16_t numSubtables = getUInt16(buffer, cmap->offset + 2);
    if (numSubtables == 0) {
    ...
    uint16_t format = getUInt16(buffer, offset);
    if (format != 4) {
        fprintf(stderr,
    ...
    uint16_t segCount = getUInt16(buffer, offset + 6) / 2;
...
void parse_cmap(CharacterMap* charMap,
    ...
    uint16_t reservedPad = getUInt16(buffer, offset + 14 + segCount * 2);
    if (reservedPad != 0) {
    ...
    for (int i = 0; i < segCount; i++) {
        uint16_t endCode = getUInt16(buffer, offset + 14 + i * 2);
        uint16_t startCode = getUInt16(buffer, offset + 16 + segCount * 2 + i * 2);
        uint16_t idDelta = getUInt16(buffer, offset + 16 + segCount * 4 + i * 2);
        uint16_t idRangeOffset = getUInt16(buffer, offset + 16 + segCount * 6 + i * 2);
